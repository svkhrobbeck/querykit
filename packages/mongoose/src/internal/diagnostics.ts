 ...
 * filter key then makes an endpoint return **more** documents than intended,
 * silently. This makes each drop either visible (`onSkipped`) or fatal (`strict`).
 *
 * Identical to the drizzle-pg adapter's `Diagnostics`, and `SkippedCondition` /
 * `QueryKitError` come from core — so both backends report identically.
 */
export interface Diagnostics {
  /** Model name (drizzle: table name), for the report. */
  source: string;
